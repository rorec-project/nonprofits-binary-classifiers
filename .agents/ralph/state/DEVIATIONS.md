...
(ORCHESTRATOR.md A.4/A.2 step 5). Rows are kept for the historical record even
after the deviation is resolved — resolved rows have a follow-up entry.
...
| 2026-06-15 | PR-2 | (fix) | Both deviations above resolved | (1) `config/smoke.yaml` uses `hf-internal-testing/tiny-random-bert`, which instantiates cleanly under transformers v5 — synthetic encoder fallback removed; (2) `_oof_probs_for_pruned()` now reads true OOF from `registry.oof_pred_probs` with `p_pos` fallback | Encoder replacement makes real smoke training possible; OOF loading connects the crossfit output to the pruned arm pipeline | Both fixes validated: `134 passed, 4 deselected`; network test 3 passed (incl. new model); slow E2E 1 passed; smoke sweep produces real checkpoints |
